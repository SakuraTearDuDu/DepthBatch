...
> 当前发布范围：`v0.1.0-alpha` 已记录并验证 DA-V2 Small 本地权重下的 PyTorch 主真实路径，以及静态方形 ONNXRuntime 部署验证路径；`fake` 仍然是 CI 与 smoke test 的安全默认入口。
...
- `onnxruntime`：部署验证路径；当前已验证的是静态方形 ONNX 导出路径
...
如果你希望看更完整的补充说明，当前仍以英文版 [docs/quickstart.md](docs/quickstart.md) 作为辅助入口。
...
| `pytorch` | alpha-supported | 已在 Windows 11 / Python 3.12.7 / `torch 2.11.0+cpu` 下，对 DA-V2 Small + 本地权重路径完成本地验证 |
| `onnxruntime` | alpha-supported | 已在 Windows 11 / Python 3.12.7 / `onnxruntime 1.24.4` 下验证静态方形导出路径 |
