    ...
    def forward(self, out_features, patch_h, patch_w):
        ...
        for index, feature in enumerate(out_features):
            if self.use_clstoken:
                feature, cls_token = feature[0], feature[1]
                readout = cls_token.unsqueeze(1).expand_as(feature)
                feature = self.readout_projects[index](torch.cat((feature, readout), -1))
            else:
                feature = feature[0]
            ...
            feature = self.resize_layers[index](feature)
            out.append(feature)
...
class DepthAnythingV2(nn.Module):
    ...
    def infer_image(self, raw_image, input_size=518):
        image, (height, width) = self.image2tensor(raw_image, input_size)
        depth = self.forward(image)
        depth = F.interpolate(depth[:, None], (height, width), mode="bilinear", align_corners=True)[
            0, 0
        ]
